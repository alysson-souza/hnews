...
5. **Interfaces and Models**: Under suffixless naming, a file whose primary identifier is `User` is named `user.ts`, matching the identifier rule above. Treat the `.model.ts` suffix as an existing-project convention to preserve (per rule 1), not as a modern default to introduce.
...
- **Models**: Name the file after its primary identifier, like any other artifact.
  ...
  - _Existing projects_: keep `user.model.ts` where that suffix is already the convention (per rule 1).
...
- **Avoid Namespace Collisions**: Without role suffixes, a component and a model for the same domain both want `user.ts`.
  - To prevent this use more specific, intent-based names for components (e.g. `class UserProfile` in `user-profile.ts` or `class UserDetail` in `user-detail.ts`) while keeping the simple domain name for the interface (`interface User` in `user.ts`).
