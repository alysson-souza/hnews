...
export class StoryItem {
  readonly story = input<HNItem | undefined>();
  readonly index = input(0);
  readonly isSelected = input(false);
  readonly loading = input(false);

  private votedItems = signal<Set<number>>(new Set());
  private storyId = computed(() => this.story()?.id);
  ...
  upvote(): void {
    ...
    if (this.hasVoted() || !story) return;

    const newVoted = new Set(this.votedItems());
    newVoted.add(story.id);
  ...
  searchByDomain(event: Event): void {
    ...
    const domain = this.getDomain(story.url);
    if (domain) {
  ...
  hasNewComments(): boolean {
    const story = this.story();
    return story ? this.visitedService.hasNewComments(story.id, story.descendants || 0) : false;
  }

  getNewCommentCount(): number {
    const story = this.story();
    return story ? this.visitedService.getNewCommentCount(story.id, story.descendants || 0) : 0;
  }
  ...
  getItemLink(): string {
    ...
    return this.locationStrategy.prepareExternalUrl(`/item/${story.id}`);
  }
  ...
  openComments(event: MouseEvent | KeyboardEvent): void {
    ...
    event.stopPropagation();
    this.sidebarService.toggleSidebar(story.id);
  }
