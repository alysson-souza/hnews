...
The outermost 24px on each side are reserved for operating-system gestures. Links, controls, editable fields, code blocks, horizontally scrollable content, and active text selections do not start discussion gestures. Vertical reading scroll, horizontal code panning, and pinch zoom stay native. Multiple touches, pointer cancellation, losing window focus, hiding the tab, resizing, rotation, or navigation cancel an unfinished gesture, including its settling animation. Reduced-motion mode removes settling and button-navigation animations.
